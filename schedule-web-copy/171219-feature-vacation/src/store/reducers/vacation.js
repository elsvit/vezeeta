...
const defaultState = {
  ...
  newFiltered: false,
  reservationInRange: [],
  reservationInRangeLoading: false,
  reservationInRangeLoaded: false,
};
...
export default (state = defaultState, { type, ...action }) => {
  ...
  case VACATION.COUNT_RESERVATIONS_IN_RANGE.LOAD: {
    const newState = Object.assign({}, state);
    newState.reservationInRangeLoading = true;
    newState.reservationInRangeLoaded = false;
    return newState;
  }
  case VACATION.COUNT_RESERVATIONS_IN_RANGE.LOAD_DONE: {
    if (action.reservationInRange) {
      const newState = { ...state };
      newState.reservationInRange = [...action.reservationInRange];
      newState.reservationInRangeLoading = false;
      newState.reservationInRangeLoaded = true;
      return newState;
    }
    return state;
  }
  case VACATION.COUNT_RESERVATIONS_IN_RANGE.LOAD_FAIL: {
    const newState = Object.assign({}, state);
    newState.reservationInRangeLoading = false;
    newState.reservationInRangeLoaded = false;
    return newState;
