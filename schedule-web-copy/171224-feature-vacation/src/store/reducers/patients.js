...
export default (state = defaultState, { type, ...action }) => {
  switch (type) {
  case PATIENTS.FETCH_PATIENT:
    return Object.assign({}, state, { currentPatientFetching: true });
  ...
  case PATIENTS.DELETE_PATIENT_DONE: {
    return Object.assign({}, state, {
      ...
      deletePatientSaving: false,
      Patients: state.searchedList.filter((patient) => patient.PatientKey !== action.patientKey),
    });
  }
