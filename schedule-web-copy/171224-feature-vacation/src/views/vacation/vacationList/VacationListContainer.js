...
import SeparateVacationDay from './VacationSeparator';
...
class VacationListContainer extends PureComponent {
   ...
   * @param (array) this.props.vacation
   * @param (array) nextProps.vacation
   * @param (array) this.props.doctorSelectedIds
   * @param (array) nextProps.doctorSelectedIds
   * @param (array) this.props.roomSelectedIds
   * @param (array) nextProps.roomSelectedIds
   */
  ...
  onClickDeleteVacation = (obj) => {
    ...
    const deleteVacantionAr = [];
    deleteVacantionAr.push(obj);
    this.setState({
      deleteVacantionAr,
    }, this.showModal());
  }
   ...
   * @param (array) doctorSelectedIds
   * @param (array) roomSelectedIds
   * @param (string) startDateIn
   * @param (string) endDateIn
   * @return (obj) sortResultVacationRow
   */
  getVacationList = (vacation, doctorSelectedIds, roomSelectedIds, startDateIn, endDateIn) => {
        ...
        for (let i2 = 0; i2 < modelLen; i2 += 1) {
          ...
          if (
            ...
            result.push({
              ...
              onClickDeleteVacation: this.onClickDeleteVacation,
            });
          }
        }
      }
    ...
    for (let i = 0; i < sortResultLen; i += 1) {
      ...
      sortResultVacationRow.push(<VacationRow
        ...
        onClickDeleteVacation={val.onClickDeleteVacation}
      />);
    }
  ...
  /*
   * submit Delete Vacation
   * @param (bool) val
   */
  submitDeleteVacation = (val) => {
    if (val) {
      this.props.deleteVacation(this.state.deleteVacantionAr);
    }
    this.setState({
      deleteVacantionAr: [],
    }, this.hideModal());
  ...
  render() {
    ...
    return (
      <VacationList
        ...
        submitDeleteVacation={this.submitDeleteVacation}
      />
    );
  }
}
